...
| 5 | **Integration** | Given valid components with all types and profile missing some fields, When selectAndRankComponents called, Then LLM processes all and returns ranked result | `components: [10 mixed items]`<br>`jobDescription: "Full 2000-char JD with multiple skills"`<br>`profile: { full_name: undefined, profession: undefined }` | ✅ `model.generateContent()` called with `model: "gemini-2.0-flash"`<br>✅ Returns complex nested JSON with ranked items | Properly structured result with all component types ranked | Profile fields default to "Not specified" in prompt, result valid |
